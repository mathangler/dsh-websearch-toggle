 ...
 *
 * The state is an ordinary registered settings namespace, and that is the whole
 * storage story on this side: the browser writes it through the settings Remote
 * with the same revision fence every other settings form uses, so there is no
 * custom route, no state file, and no second source of truth. What this module
 * owns is only the part that has to run on the Host: adopting the committed
 * value and projecting it onto the prompt.
 *
...
/**
 * The settings namespace this plugin's switch lives in.
 *
 * On DSH 0.1.7 a namespace IS the Loader entry id, so this is the host row's id
 * from `cordis.patch.yml` — not a name the plugin chooses.
 */
export const NAMESPACE = 'websearch-toggle';
...
export function createCore(options) {
  ...
  function deny(name) {
    ...
    return 'Web search is turned off. Tell the user it is disabled on the Plugins page instead of retrying; web_fetch still works.';
  }
...
/**
 * The method map a route or a test dispatches onto.
 *
 ...
 * This package ships no route of its own — the browser reaches these through
 * the platform's own settings Remote — so the map exists for tests and for any
 * host that wants to expose one.
 *
