 ...
 * registers. Reads and writes both go through `ctx.settingsScope.bind(...)` —
 * the SAME bound scope every shipped settings form uses. That matters: the scope
 * tracks the namespace revision itself and re-reads after its own writes, so
 * this bundle never has to guess a revision. An earlier build called
 * `ctx.remote.settings.update` directly with a revision it had cached, and the
 * document's fence refused the second write, which left the switch stuck.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * The bound settings scope this switch reads and writes.
     *
     * `settingsScope.bind({ namespace })` returns the same per-namespace scope
     * every shipped settings form uses. Its snapshot carries `status` (`ready`
     * once the mirror holds the namespace), the resolved `value`, and `writable`;
     * its `set(field, value)` queues the write with the revision the scope
     * itself is tracking, then re-reads. Nothing here manages a revision, and
     * nothing here touches the Remote directly — that is the point.
     *
    ...
    function settingsPort(ctx) {
      ...
      let scope = null;
      try {
        scope = service !== null && typeof service.bind === 'function' ? service.bind({ namespace: NS }) : null;
      } catch {
        scope = null;
      }

      /** Read the namespace's current scope snapshot. */
      function read() {
    ...
    /** Required client services: the locale for copy, the settings scope to bind. */
