...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Flip the switch: optimistic, then reconciled against what the host stored.
     *
     * @param ui - the shared plugin state.
     */
    async function flip(ui) {
      if (ui.busy || !ui.known || ui.readOnly) return;
      ...
      const answer = await ui.port.write(ui.enabled, revision);
      ui.busy = false;
      if (!answer.ok) {
        ui.enabled = previous;
      } else {
        const reread = ui.port.read();
        if (reread.ok) {
          ui.enabled = reread.value.enabled;
          ui.revision = reread.value.revision;
        }
      }
      paint(ui);
    }
