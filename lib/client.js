 ...
 *
 * State lives in this plugin's own settings namespace — the Loader entry id
 * `websearch-toggle` — which 0.1.7 projects from the host half's `Config` schema
 * automatically. Reads and writes both go through the per-namespace settings
 * form, `ctx.configForms.get(namespace)`, the same object every shipped settings
 * form uses (0.1.6 called it `settingsScope.bind({ namespace })`).
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Settings namespace for the switch.
     *
     * On 0.1.7 a namespace IS the Loader entry id, so this is the host row's id
     * (`websearch-toggle`), NOT a name this bundle gets to choose. It used to be
     * `web-search-toggle`, invented when the host half registered its own
     * settings section — a mechanism 0.1.7 removed.
     */
    const NS = 'websearch-toggle';
